...
impl Doer for SvcTempEntity {
    ...
    fn parse(i: &[u8], aux: AuxRefCell) -> Result<Self> {
        ...
        let (i, entity) = match entity_type {
            ...
            30 => map(take(17usize), |res: &[u8]| TempEntity::TeLine(res.to_vec())).parse(i)?,
            31 => map(take(17usize), |res: &[u8]| TempEntity::TeBox(res.to_vec())).parse(i)?,
            ...
            104 => map(take(9usize), |res: &[u8]| TempEntity::TeDecal(res.to_vec())).parse(i)?,
            105 => map(take(5usize), |res: &[u8]| TempEntity::TeFizz(res.to_vec())).parse(i)?,
            ...
            _ => context("Bad temp entity number", fail()).parse(i)?,
        };
